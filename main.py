...
def get_answer_index(q: Question):
    return list(map(lambda o: o.prefix, q.answer.options)).index("=")
...
'''
        ...
        for i, q in enumerate(questions):
            ...
            options_text = map(lambda o: o.raw_text[1:], q.answer.options)
...
def get_args():
    ...
    parser.add_argument("--gift_path", required=True, type=str, help="Path to GIFT file")

    limit_time = lambda x: x if x in range(1, 301) else 300

    parser.add_argument("--output", "-o", type=str, default="blooket.csv", help="Output path for CSV")
    parser.add_argument("--delimiter", "-d", default=",", type=str, help="Delimiter for separating values")
    parser.add_argument("--time", "-t", default=300, type=limit_time, help="Time Limit (sec) (Max: 300 seconds)")
